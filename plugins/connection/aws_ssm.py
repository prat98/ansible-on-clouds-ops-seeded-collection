# This code is part of Ansible on Clouds, but is an independent component.
# This particular file, and this file only, is based on
# plugins/connection/aws_ssm.py of community.aws Ansible Collection
#
...
class Connection(ConnectionBase):
    ...
    def _get_url(
        self, client_method, bucket_name, out_path, http_method, extra_args=None
    ):
    ...
    def _get_boto_client(
        self, service, region_name=None, profile_name=None, endpoint_url=None
    ):
    ...
    def _generate_encryption_settings(self):
        ...
        if self.get_option("bucket_sse_mode") == "aws:kms" and self.get_option(
            "bucket_sse_kms_key_id"
        ):
    ...
    def _generate_commands(self, bucket_name, s3_path, in_path, out_path):
        ...
        put_url = self._get_url(
            "put_object", bucket_name, s3_path, "PUT", extra_args=put_args
        )
        ...
        if self.is_windows:
            ...
            get_commands = [
                ("Invoke-WebRequest " f"'{get_url}' " f"-OutFile '{out_path}'"),
            ]
        else:
            put_command_headers = " ".join(
